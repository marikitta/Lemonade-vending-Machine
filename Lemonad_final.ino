...
void drink_sparkling(){ 
    ...
    Serial.println(hx1.read()/1000); //до 8555
    Serial.print("Наливается в стакан  ");
    Serial.println(count);
    Serial.println(add[count]);
    long weight=((sensors[count]->read())/1000)+add[count];
    bool flag=false;
    Serial.println((stabelweight(sensors[count])/1000)+add[count]);
    while (flag==false){
    if (pouring[count]==false){
        if (((stabelweight(sensors[count])/1000)+add[count])>8450 ) {
//      if (weight>8350){
        Serial.println("Водичка пошла");
        digitalWrite(klapan1_PIN,LOW);
        pouring[count]=true;
      ...
      long weight=(stabelweight(sensors[count])/1000)+add[count];
       Serial.println(weight);
      if (weight<8455){
        digitalWrite(klapan1_PIN,HIGH);
        Serial.println("Водичка не идет нет стакана");
        pouring[count]=false;
      }
      
      else if (weight>=8507) {
        Serial.println("Водичка не идет наполнили");
        digitalWrite(klapan1_PIN,HIGH);
        pouring[count]=false;
        flag=true;
      }
...
void drink_mint(){
    ...
    Serial.println(hx1.read()/1000); //до 8545
   bool flag=false;
    Serial.println((stabelweight(sensors[count])/1000)+add[count]);
    while (flag==false){
    if (pouring[count]==false){
      ...
      if (weight<8455){
        digitalWrite(klapan3_PIN,HIGH);
        Serial.println("Мята не идет нет стакана");
        pouring[count]=false;
      }
      
      else if (weight>=8467) {
        Serial.println("Мята не идет наполнили");
        digitalWrite(klapan3_PIN,HIGH);
        pouring[count]=false;
        flag=true;
      }
...
void drink_juice(){
    ...
    Serial.println(hx1.read()/1000); //до 8515
    bool flag=false;
    Serial.println((stabelweight(sensors[count])/1000)+add[count]);
    while (flag==false){
    if (pouring[count]==false){
      ...
      if (weight<8455){
        digitalWrite(klapan2_PIN,HIGH);
        Serial.println("Сок не идет нет стакана");
        pouring[count]=false;
      }
      
      else if (weight>=8497) {
        Serial.println("Сок не идет наполнили");
        digitalWrite(klapan2_PIN,HIGH);
        pouring[count]=false;
        flag=true;
      }
...
void cocktail_mint(){ 
    ...
    digitalWrite(klapan3_PIN,HIGH);
    Serial.println("Тархун не пошел");
    Serial.println(hx1.read()/1000); //до 8605    в. до 8585 + м. до 8605
    bool flag=false;
    //Serial.println((stabelweight(sensors[count])/1000)+add[count]);
    while (flag==false){
    if (pouring[count]==false){
      ...
      if (weight<8455){
        digitalWrite(klapan1_PIN,HIGH);
        Serial.println("Вода не идет нет стакана");
        pouring[count]=false;
      }
      
      else if (weight>=8537) {
        Serial.println("Вода не идет наполнили");
        digitalWrite(klapan1_PIN,HIGH);
        pouring[count]=false;
        flag=true;
      }
...
void cocktail_orange(){
    ...
    digitalWrite(klapan2_PIN,HIGH);
    Serial.println("Сок не пошел");
    Serial.println(hx1.read()/1000); //до 8585    в. 8535 + а. 8585
    bool flag=false;
    //Serial.println((stabelweight(sensors[count])/1000)+add[count]);
    while (flag==false){
    if (pouring[count]==false){
...
void cocktail_triple(){ 
    ...
    Serial.println(hx1.read()/1000); //до 8595    в. 8540 + а. 8585 + м. 8595
    bool flag=false;
    Serial.println((stabelweight(sensors[count])/1000)+add[count]);
    while (flag==false){
    if (pouring[count]==false){
